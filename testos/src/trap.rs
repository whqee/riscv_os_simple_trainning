...
pub fn init() {
    ...
    cpu::set_stvec_direct_mode(__s_trap_entry as usize);
}

#[no_mangle]
// Called by trap.S: __s_trap_entry
pub fn s_trap_handler(frame: &mut RegFrame) {
    // println!("mcause: 0x{:x}", cpu::Mcause::mcause());
    // println!("mip: 0x{:x}", cpu::mip());
    // cpu::set_mip(0);

    // Simple trace log
    let cause = cpu::Scause::scause().into();
    // let mpp: cpu::MPP = cx.frame.sstatus.into();
    print!(
        "[M][Trap] {:?}, sepc=0x{:x}, sepc=0x{:x} ",
        cause,
        cpu::sepc(),
        frame.sepc
    );
            ...
            cpu::Exception::ECallFromU => {
                ...
                match frame.a7.into() {
                    ...
                    ECall::SwitchToContext => unsafe {
                        ...
                        print!(
                            ", switching to context {{ sepc=0x{:x} ra=0x{:x} Mode={:?} satp:0x{:x} }}",
                            frame.sepc,
                            frame.ra,
                            cpu::SPP::from(frame.sstatus),
                            frame.satp
                        );
                    },
                    _ => todo!(),
