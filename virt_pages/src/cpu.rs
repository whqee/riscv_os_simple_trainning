...
impl SPP {
    ...
    #[inline]
    pub fn set(spp: SPP) {
        SStatus::write(SStatus::read().raw & !(1usize << 8) | ((spp as usize) << 8))
    }
}
...
impl From<SStatus> for SPP {
    fn from(sstatus: SStatus) -> Self {
        match (sstatus.raw >> 11) & 3 {
            0 => SPP::User,
            1 => SPP::Supervisor,
            _ => panic!("Unsupported 'SPP' code at 'sstatus'."),
        }
...
impl From<usize> for ExceptionCause {
    fn from(cause: usize) -> Self {
        ...
        if (cause >> (core::mem::size_of::<usize>() - 1)) == 0 {
            // Exception
            match cause {
            ...
            // Interrupt
            match cause & !(1 << (core::mem::size_of::<usize>() - 1)) {
...
impl From<Mcause> for ExceptionCause {
    fn from(value: Mcause) -> Self {
        value.mcause.into()
    }
}

impl From<Scause> for ExceptionCause {
...
#[inline]
pub(crate) fn enable_s_interrupt() {
    SStatus::write(SStatus::read().raw | (1 << 3))
}
...
#[inline]
pub fn set_stvec_direct_mode(trap_handler: usize) {
    unsafe {
...
#[inline]
pub fn set_sscratch(sscratch: usize) {
    unsafe {
        asm!("csrw sscratch, {0}", in(reg)(sscratch));
    }
}
...
macro_rules! sret {
    () => {{
        unsafe { core::arch::asm!("sret", options(noreturn)) }
    }};
}
